...
description: "Seedance 2.0 微短剧分镜全局规则体系，涵盖视觉渲染、镜头拆分、空间连续性、景别衔接、运镜、表演、潜台词、群像、声音设计、提示词优化等 19 项规则。Invoke when 用户进行微短剧分镜创作、镜头拆分、Seedance 2.0 提示词编写、电影级 CG 分镜设计，或需要套用固定分镜模板时。"
---
...
本 Skill 定义了使用 Seedance 2.0 进行微短剧分镜创作的全套全局规则，共 19 节，覆盖视觉渲染、镜头拆分、空间连续性、景别衔接、运镜、表演、潜台词、群像、声音设计、提示词优化等全链路要素。
...
19. **`prompt-optimization.md`** — 十九、提示词优化规则（保留情绪表达、逻辑重音标注、潜台词与台词区分）。
20. **`workflow.md`** — 使用工作流（何时调用、如何套用模板、如何校验、输出规范）。
